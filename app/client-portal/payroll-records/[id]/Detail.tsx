...
import { buildDetailedSheet, collectCustomNames, type DetailedRecord } from "@/lib/payrollExcel";
...
type PayrollRunApproval = { status?: string; preparedByName?: string; checkedByName?: string; approvedByName?: string; approvedAt?: string; };
...
function groupRecords(records: PayrollRecord[]): RunGroup[] {
  ...
  for (const r of records) {
    if (r.archiveStatus === "Archived") continue; // archived runs are not visible to clients
...
async function exportRunToExcel(group: RunGroup, approvalStatus: string, companyName: string) {
...
export default function ClientPortalPayrollRunDetail() {
  ...
  const [approval, setApproval] = useState<PayrollRunApproval>({});
  ...
  useEffect(() => {
    async function load() {
      ...
      const [themeRaw, records, approvals, companyInfo] = await Promise.all([
        ...
        getConfigItem<{ companyName?: string } | null>(storageKeys.companyInformation, null),
      ]);
  ...
  return (
    <div className="min-h-screen pg-bg">
      ...
      <section className="relative overflow-hidden border-b px-6 py-5 shadow-[0_22px_60px_-42px_rgba(14,165,233,0.75)]"
        ...
        <div className="pointer-events-none absolute inset-0" style={{ backgroundImage: `radial-gradient(circle at 82% 20%, ${theme.accentColor}33, transparent 30%)` }} />
        ...
        <div className="relative mx-auto max-w-7xl">
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <h1 className="text-2xl font-semibold tracking-tight" style={{ color: theme.bannerTextColor }}>Payroll Summary</h1>
              <p className="mt-1 text-sm opacity-75" style={{ color: theme.bannerTextColor }}>
                {group.title} · {group.records.length} employees · {group.period} · {fmtDate(group.date)}
              </p>
            </div>
            <button type="button"
              onClick={() => exportRunToExcel(group, status, companyName).catch(console.error)}
      ...
      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6">
        ...
        <div className="mb-4 flex flex-wrap items-center gap-3">
          ...
          {approval.approvedByName && (
            <span className="text-sm text-slate-500">Approved by {approval.approvedByName}{approval.approvedAt ? ` · ${fmtDate(approval.approvedAt)}` : ""}</span>
          )}
        </div>
        ...
        {/* Detailed register — identical to the admin view and the exported Excel. */}
        <DetailedPayrollRegister
          records={filteredRecords as unknown as DetailedRecord[]}
          period={group.period}
          payDate={group.date}
        />
        <div className="mt-2 text-xs text-slate-400">
          Read-only view · No modifications allowed from the client portal
        </div>

        {/* Legacy summary table retained but hidden; the detailed register above replaces it. */}
                  ...
                  pageRecords.map((r, i) => {
                    ...
                    return (
                      <tr key={r.id || r.employeeNo || i} className="border-b border-slate-100 hover:bg-slate-50/50">
                        <td className="whitespace-nowrap border-r border-slate-100 px-3 py-2.5 text-center text-xs text-slate-400">{globalIdx}</td>
              ...
              {filteredRecords.length > 0 && page === totalPages && (
                <tfoot>
                  <tr className="border-t border-slate-200 bg-slate-50/70">
                    <td colSpan={7} className="whitespace-nowrap border-r border-slate-100 px-3 py-3 text-sm font-semibold text-slate-700">
                      Total ({group.records.length} employees)
                    </td>
